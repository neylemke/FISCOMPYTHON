...
static PyObject* test(PyObject*self, PyObject* args, PyObject* kywds)
{
    ...
    static char *kwlist[] = {"a","b","local_dict", NULL};
    PyObject *py_a, *py_b;
    int a_used, b_used;
    py_a = py_b = NULL;
    a_used= b_used = 0;
    
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"OO|O:test",kwlist,&py_a, &py_b, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_a = py_a;
        ...
        a_used = 1;
        py_b = py_b;
        std::string b = convert_to_string(py_b,"b");
        b_used = 1;
        /*<function call here>*/     
...
PyMODINIT_FUNC initext_string_and_int(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("ext_string_and_int", compiled_methods);
}
