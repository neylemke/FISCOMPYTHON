...
static PyObject* compiled_func(PyObject*self, PyObject* args)
{
    ...
    PyObject *py_a, *py_key, *py_val;
    py_a = py_key = py_val = NULL;
    ...
    {                                
        ...
        py::dict a = convert_to_dict(py_a,"a");
        py_key = get_variable("key",raw_locals,raw_globals);
        std::complex<double> key = convert_to_complex(py_key,"key");
        py_val = get_variable("val",raw_locals,raw_globals);
        int val = convert_to_int(py_val,"val");
        /* inline code */                   
        /* NDARRAY API VERSION 1000009 */
        a[key] = val;    /*I would like to fill in changed    locals and globals here...*/   
...
PyMODINIT_FUNC initsc_c74ad97099f7d86a4efefddc19b22a010(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("sc_c74ad97099f7d86a4efefddc19b22a010", compiled_methods);
}
