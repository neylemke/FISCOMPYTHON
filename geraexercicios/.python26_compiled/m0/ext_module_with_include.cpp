...
static PyObject* test(PyObject*self, PyObject* args, PyObject* kywds)
{
    ...
    if(!PyArg_ParseTupleAndKeywords(args,kywds,"O|O:test",kwlist,&py_a, &py_local_dict))
       return NULL;
    try                              
    {                                
        py_a = py_a;
...
PyMODINIT_FUNC initext_module_with_include(void)
{
    
    Py_Initialize();
    import_array();
    PyImport_ImportModule("numpy");
    (void) Py_InitModule("ext_module_with_include", compiled_methods);
}
